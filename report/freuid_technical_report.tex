...
\newcommand{\teamname}{test\_freuid}
\newcommand{\kaggleteam}{test\_freuid}
\newcommand{\contactemail}{semenoffm48@gmail.com}
...
\textbf{Code:} \url{https://github.com/msemenoff/freuid-challenge-2026} (commit \texttt{d3a0b9e594a28786c6c8859b81d638608676da98})\\
...
oversampling the 20 real captured rows 200$\times$ within each training epoch
was run late in development and produced a checkpoint, but its public-LB
score was never confirmed before the July 13 code freeze took effect. It was
therefore \emph{not} selected: the final submission uses the original
capture-augmentation-only champion checkpoint (no oversampling), consistent
with the code-freeze requirement that model selection be locked in before
the private test release.
...
at scale 736 without TTA. Private-set inference (134{,}997 images, same GPU
and recipe) completed in 4h50m32s (7.74 images/s steady state), run
inference-only on July 13, 2026 per the competition's code-freeze policy.
CPU-only wall-clock was not benchmarked.
...
Private leaderboard: final submission (\texttt{submission\_FINAL\_private\_merged.csv},
uploaded 2026-07-13 12:04 PM PST / 19:04 UTC) scored \textbf{0.00465} at
submission time, matching our public-leaderboard optimum. This file merges
the 7{,}821 real public predictions with 134{,}997 real private predictions
produced by an inference-only run of the frozen champion checkpoint against
the private test images released July 13, 2026, with no fallback values
remaining. The competition platform may report a private-only component
separately after final close; this report will not be revised after the
July 15 deadline per the code-freeze policy.
...
\label{sec:repro}
\begin{itemize}[noitemsep]
  \item \textbf{Repository:} \url{https://github.com/msemenoff/freuid-challenge-2026},
  commit \texttt{d3a0b9e594a28786c6c8859b81d638608676da98}. This is the
  commit whose code (docker/prepare\_submission.py, src/, config.yaml) was
  actually used to run the inference-only private-test scoring on July 13,
  2026; only documentation/reply-template commits were added afterward, and
  model weights (\texttt{models/convnext\_full\_drop01\_rect832\_capaug\_e1.pth})
  were never modified after the July 4, 2026 push, satisfying the
  competition's code-freeze requirement.
  ...
  \item \textbf{Runtime:} single 16\,GB GPU recommended (private-set
  inference: 134{,}997 images in 4h50m32s, 7.74 images/s); CPU fallback is
  supported but was not benchmarked.
...
Solo entry. Michael Semenoff (team \teamname) performed all modeling,
experimentation, data analysis, engineering (training pipeline, Docker
reproducibility sandbox), and writing.

\section*{Acknowledgments}
No external funding or compute grants to disclose. Thanks to the FREUID
Challenge 2026 organizers for a well-specified reproducibility contract.
